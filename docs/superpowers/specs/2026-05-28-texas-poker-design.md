...
8. 玩家可执行 `check`、`call`、`fold`、`bet`、`raise`、`all-in`，服务端根据当前下注额、最小加注额、玩家筹码和阶段判断是否合法。
9. 若只剩一名未弃牌玩家，本手牌立即结束并把全部可赢底池分给该玩家。
10. 若 river 后仍有多名未弃牌玩家，进入摊牌，使用每人两张手牌和五张公共牌组成最佳五张牌比较胜负。
11. 支持 all-in 和边池。玩家只能赢取自己有资格参与的底池；同牌型平分，无法整除的零头按按钮左侧顺时针顺序分配。
...
牌桌 UI 包含玩家座位、公共牌、底池、当前阶段、当前行动玩家、自己的手牌、操作按钮和房主工具。
...
玩家可向房主提交补码申请，包含申请筹码数量。房主批准后：

- 增加玩家累计补码 `totalInvested`。
- 若当前没有进行中的手牌，立即增加当前筹码。
- 若当前手牌进行中，补码记为待生效，在下一手牌开始前加入当前筹码。
